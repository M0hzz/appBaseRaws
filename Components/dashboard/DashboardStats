...
export default function DashboardStats({ moodEntries, marketNews, patterns, isLoading }) {
  ...
  const getPositiveNewsPercentage = () => {
    ...
    const positiveNews = marketNews.filter(news => news.sentiment_score > 0).length;
